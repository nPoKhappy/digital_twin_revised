...
def main(config_path: str):
    ...
    W = int(config['window']['train_window_mins'] / config['window']['sampling_interval_min'])
    H_ss = H_out
    gain_num_ar_blocks = len(config['training'].get('loss_weighting', {}).get('weights', [1]))
    if gain_num_ar_blocks < 1:
        raise ValueError("training.loss_weighting.weights must contain at least one block weight.")
    H_gain = H_out * gain_num_ar_blocks
    ...
    dynamic_gain_step_change_block = int(config['training'].get('dynamic_gain_step_change_block', gain_num_ar_blocks))
    if dynamic_gain_step_change_block < 1 or dynamic_gain_step_change_block > gain_num_ar_blocks:
        raise ValueError(
            "dynamic_gain_step_change_block must be 1-based and within the configured AR block count."
        )
    dynamic_gain_step_change_block_idx = dynamic_gain_step_change_block - 1
    dynamic_gain_tail_start_local_idx = min(dynamic_gain_tail_start_step - 1, H_out - 1)
    dynamic_gain_tail_start_idx = dynamic_gain_step_change_block_idx * H_out + dynamic_gain_tail_start_local_idx
    dynamic_gain_tail_end_idx = (dynamic_gain_step_change_block_idx + 1) * H_out
    ...
    print(
        ...
        f"Dynamic gain rollout: {gain_num_ar_blocks} AR blocks x {H_out} steps = {H_gain} steps; "
        f"Step change block: {dynamic_gain_step_change_block}/{gain_num_ar_blocks}; "
        f"Dynamic gain output: block-local steps {dynamic_gain_tail_start_local_idx + 1}-{H_out} "
        f"(global steps {dynamic_gain_tail_start_idx + 1}-{dynamic_gain_tail_end_idx}); "
        ...
        for step, mse_batch in pbar:
            ...
            if compute_gain_metrics:
                ...
                def rollout_gain_stack(base_de_p_tensor, step_de_p_tensor=None, collect_debug=False):
                    ...
                    gain_log_blocks_local = []
                    debug_steps_local = []
                    for block_idx in range(gain_num_ar_blocks):
                        current_de_z = (
                            step_de_z_graph
                            if block_idx == dynamic_gain_step_change_block_idx
                            else base_de_z_graph
                        )
                        de_block = current_de_z.unsqueeze(1).expand(-1, H_out, -1)
                        pred_z_gain = dynamic_model(gain_history, de_block)
                        pred_log_or_p_gain = pred_z_gain * y_std_safe + y_mean_tensor
                        gain_log_blocks_local.append(pred_log_or_p_gain[:, :, tab_target_idx])
                        ...
                        if collect_debug:
                            ...
                            for t_debug in range(pred_p_gain.shape[1]):
                                debug_steps_local.append(pred_p_gain[:, t_debug, tab_target_idx].detach())

                        new_block = torch.cat([de_block, pred_z_gain], dim=2)
                        gain_history = torch.cat([gain_history[:, H_out:, :], new_block], dim=1)

                    gain_log_stack_local = torch.cat(gain_log_blocks_local, dim=1).transpose(0, 1)
