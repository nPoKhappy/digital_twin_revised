...
OUTPUT_PERTURB_FILENAME = "LHS/LHS_air2_t2_soft_ood_perturbation.csv"
...
SAFE_INNER_AIR2_RANGE = (142.5, 297.5)
SAFE_INNER_T2_RANGE = (142.5, 237.5)
AIR2_LOW_RANGE = (110.0, 140.0)
AIR2_HIGH_RANGE = (300.0, 340.0)
T2_LOW_RANGE = (125.0, 140.0)
T2_HIGH_RANGE = (240.0, 250.0)

# Keep the three acid-gas conditions near the Dynamics training center while
# air2 and/or T2 are softly extrapolated.
SAFE_ACIDGAS_RANGES = {
...
def make_strata():
    """Return 400 single-OOD bases and 100 joint-OOD bases."""
...
def validate_soft_ood(df_output):
    if len(df_output) != 5000:
        raise ValueError(f"Expected exactly 5000 rows, got {len(df_output)}.")

    air2_outside = (
        (df_output[AIR2_TAG] <= CORE_AIR2_RANGE[0])
        | (df_output[AIR2_TAG] >= CORE_AIR2_RANGE[1])
    )
    t2_outside = (
        (df_output[T2_TAG] <= CORE_T2_RANGE[0])
        | (df_output[T2_TAG] >= CORE_T2_RANGE[1])
    )
    if not (air2_outside | t2_outside).all():
        raise ValueError("At least one generated row fell completely inside the core.")

    single_count = int((air2_outside ^ t2_outside).sum())
    joint_count = int((air2_outside & t2_outside).sum())
    if single_count != 4000 or joint_count != 1000:
        raise ValueError(
            "Unexpected soft-OOD composition: "
...
def main():
    ...
    single_count, joint_count = validate_soft_ood(df_output)
    df_output.to_csv(OUTPUT_PERTURB_FILENAME, index=False)

    plot_dir = os.path.join(base_dir, "LHS_Air2_T2_Soft_OOD_Check_Plots")
    ...
    print("Done.")
    print(f"Soft-OOD rows: {len(df_output)} -> {OUTPUT_PERTURB_FILENAME}")
    print(f"Single-variable OOD rows: {single_count}")
    print(f"Joint air2 x T2 OOD rows: {joint_count}")
